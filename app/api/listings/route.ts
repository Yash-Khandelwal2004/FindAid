...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      apiError("Failed to fetch listings"),
      { status: 500 }
    )
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      quantity, unit, condition, isUrgent,
      images, location, availableFrom, availableTill, tags,
    } = body

    if (!title || !description || !category || !quantity || !condition || !location) {
      return NextResponse.json(
        apiError("title, description, category, quantity, condition and location are required"),
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      apiError("Failed to create listing"),
      { status: 500 }
    )
  }
}